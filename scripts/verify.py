#!/usr/bin/env python3
"""Playwright-based UX verification for a lazy-data2web static site.
...
# Attribution markers that must never reach a shipped site (see tech-stack.md §11
# "No tool attribution"): the deliverable is the user's site, not an advert for the
# toolchain, so it must not disclose it was produced by this skill / a generator.
ATTRIBUTION_TOKENS = ("lazy-data2web", "built with lazy", "generated by lazy")
...
def check_no_attribution(page: Page, results: Results) -> None:
    ...
    # comments — the places a skill name most plausibly leaks into output.
    html = (page.content() or "").lower()
    hits = sorted({tok for tok in ATTRIBUTION_TOKENS if tok in html})
    if hits:
        results.fail(
            "No tool attribution in the shipped page",
            f"found {', '.join(hits)} — the site must not reveal it was built with this skill",
        )
    else:
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Verify a lazy-data2web static site's UX with Playwright.",
    )
